...
// SyncWrite Goal_Current for 2 joints
#define SYNCWRITE_CUR_LEN 20

// USB serial packet: 2-byte magic + seq(u8) + 7×float32 = 31 bytes
#define PKT_MAGIC_0  0xAA
#define PKT_MAGIC_1  0x55
#define PKT_PAYLOAD  (1 + N_MOTORS * 4)  // seq + 7 floats
...
// ── Low-level write (no response expected) ────────────────────────────────────
void writeReg(uint8_t id, uint16_t addr, uint8_t *data, uint8_t dlen) {
    ...
    DxlSerial.flush();
    delay(5);
    while (DxlSerial.available()) DxlSerial.read();
}
...
// positions[0..5] → radians, positions[6] (gripper) → raw ticks
bool syncReadPositions(float *positions) {
    ...
    uint32_t deadline = millis() + 50;  // 50 ms total for all 7 responses

    for (int m = 0; m < N_MOTORS; m++) {
        int rx = 0;
        while (rx < STATUS_PKT_LEN && millis() < deadline) {
            if (DxlSerial.available()) buf[rx++] = DxlSerial.read();
            else yield();  // let TinyUSB run while waiting for UART bytes
        }
        if (rx < STATUS_PKT_LEN) return false;
        if (buf[0] != 0xFF || buf[1] != 0xFF || buf[2] != 0xFD ||
            buf[3] != 0x00 || buf[7] != 0x55) return false;
        ...
        if (idx < N_MOTORS - 1) {
            positions[idx] = (ticks / 4096.0f) * 2.0f * M_PI - M_PI;  // radians
        } else {
            positions[idx] = (float)ticks;  // gripper: raw ticks, Python normalises
        }
    }
    return true;
}

// ── SyncWrite Goal_Current to joints 3 & 4 (pushback spring) ─────────────────
...
void configureDynamixel() {
    ...
    // torque stays off until CMD_START
...
void setup() {
    ...
    DxlSerial.begin(DXL_BAUD, SERIAL_8N1, /*RX=*/DXL_PIN, /*TX=*/DXL_PIN);
    uart_set_mode(UART_NUM_1, UART_MODE_RS485_HALF_DUPLEX);

    delay(3000);  // let USB CDC enumerate before any motor traffic
...
// ── Loop ─────────────────────────────────────────────────────────────────────
void loop() {
    // auto-stop if USB host disconnects
    if (state == RUNNING && !Serial) {
        write2(ID_JOINT3, ADDR_GOAL_CURRENT, 0);
        write2(ID_JOINT4, ADDR_GOAL_CURRENT, 0);
        write1(ID_GRIPPER, ADDR_TORQUE_ENABLE, 0);
        state = WAITING;
    }

    // handle commands
    static bool awaitingCmd = false;
    while (Serial.available()) {
        uint8_t b = Serial.read();
        if (!awaitingCmd) {
            if (b == CMD_MAGIC) awaitingCmd = true;
        } else {
            awaitingCmd = false;
            if (b == CMD_START && state == WAITING) {
                write1(ID_GRIPPER, ADDR_TORQUE_ENABLE, 1);
                write4(ID_GRIPPER, ADDR_GOAL_POSITION, GRIPPER_OPEN_POS);
                state = RUNNING;
            } else if (b == CMD_STOP && state == RUNNING) {
                write2(ID_JOINT3, ADDR_GOAL_CURRENT, 0);
                write2(ID_JOINT4, ADDR_GOAL_CURRENT, 0);
                write1(ID_GRIPPER, ADDR_TORQUE_ENABLE, 0);
                state = WAITING;
    ...
    static bool          rrPrinted  = false;

    if (!rrPrinted && Serial) {
        const char* rr_str = "unknown";
        switch (g_reset_reason) {
            case ESP_RST_POWERON:  rr_str = "power-on";      break;
            case ESP_RST_SW:       rr_str = "software";      break;
            case ESP_RST_PANIC:    rr_str = "panic";         break;
            case ESP_RST_INT_WDT:  rr_str = "int-watchdog";  break;
            case ESP_RST_TASK_WDT: rr_str = "task-watchdog"; break;
            case ESP_RST_WDT:      rr_str = "watchdog";      break;
            case ESP_RST_BROWNOUT: rr_str = "brownout";      break;
            case ESP_RST_USB:      rr_str = "usb-watchdog";  break;
            default: break;
        }
        Serial.printf("reset_reason=%s(%d) heap=%lu\n", rr_str, (int)g_reset_reason, ESP.getFreeHeap());
        rrPrinted = true;
    }
    ...
    if (millis() - last < 20) { delay(1); return; }  // yield to idle/TinyUSB between ticks
    last = millis();

    float positions[N_MOTORS] = {};
    if (!syncReadPositions(positions)) {
        failCount++;
        if (failCount == 1 || failCount % 10 == 0) {
            Serial.printf("SyncRead fail #%lu\n", failCount);
        }
        return;  // follower holds last position
    }
    failCount = 0;

    applyPushback(positions);

    // Binary packet: 0xAA 0x55 | seq(u8) | 7×float32 LE  →  31 bytes total
    uint8_t pkt[2 + PKT_PAYLOAD];
    pkt[0] = PKT_MAGIC_0;
    pkt[1] = PKT_MAGIC_1;
    pkt[2] = seq++;
    memcpy(pkt + 3, positions, N_MOTORS * sizeof(float));
